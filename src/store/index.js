	...
	//商品信息
		{
			...
			flag:false,
			num:1,
			description:'涵盖并行软件和硬件的方方面面，手把手教你如何利用MPI、PThread 和OpenMP开发高效的并行程序',
			sizes:['S','M','L','XL'],
			size:'',
			colors:['黑色','白色','红色','灰色'],
			color:'',
...
const mutations={
	...
	CHANGE_FLAG_TURE(state,index){
		state.books[index].flag=true
	},
	CHANGE_FLAG_FALSE(state,index){
		state.books[index].flag=false
	},
  CHANGE_BOOK_FLAG_TRUE(state,index){
    state.books[index].flag=true
  },
  CHANGE_BOOK_FLAG_FALSE(state,index){
    state.books[index].flag=false
  },
...
const actions={
  ...
  changeBookFlagTrue({commit},index){
    commit('CHANGE_BOOK_FLAG_TRUE',index)
  },
  changeBookFlagFalse({commit},index){
    commit('CHANGE_BOOK_FLAG_FALSE',index)
  },
	changeSelectedItem({commit},index){
		commit('CHANGE_SELECTED_ITEM',index)
	},
  changeSelectedBookItem({commit},index){
    commit('CHANGE_SELECTED_BOOK_ITEM',index)
  },
